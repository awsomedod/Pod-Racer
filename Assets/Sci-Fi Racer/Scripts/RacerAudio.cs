...
//		[SerializeField] private float m_EngineMinThrottlePitch = 0.4f;       // Pitch of the engine sound when at minimum throttle.
	...
	//	[SerializeField] private float m_EngineFwdSpeedMultiplier = 400f;   // Additional multiplier for an increase in pitch of the engine from the plane's speed.
		...
		private void Update()
		{
			...
			//m_EngineSoundSource.pitch = gameObject.GetComponent<HoverCarControl>().m_currThrust/m_EngineFwdSpeedMultiplier;
			m_EngineSoundSource.pitch = m_WindBasePitch + planeSpeed*(m_WindSpeedPitchFactor*2);
